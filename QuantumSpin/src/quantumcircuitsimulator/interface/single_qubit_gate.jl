...
# XZQ add
struct AmpRzGate{T} <: AbstractOneBodyGate
	positions::Tuple{Int}
	parameter::T
	amp::Int
end
AmpRzGate(pos::Int, p, a) = AmpRzGate((pos,), p, a)
op(s::AmpRzGate) = AmpRz(value(s.parameter))
shift(s::AmpRzGate, i::Int) = AmpRzGate(_shift(positions(s), i), s.parameter, s.amp)
scalar_type(::Type{AmpRzGate{T}}) where {T<:Real} = ComplexF64
